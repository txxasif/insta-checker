...
async function checkViaHtmlPage(username: string): Promise<CheckResult | null> {
  ...
  // Extract from og:description or description meta tag
  const descRegex = /<meta\s+(?:[^>]*property=["']og:description["'][^>]*content=["']([^"']+)["']|[^>]*content=["']([^"']+)["'][^>]*property=["']og:description["']|[^>]*name=["']description["'][^>]*content=["']([^"']+)["']|[^>]*content=["']([^"']+)["'][^>]*name=["']description["'])/i;
  const descMatch = html.match(descRegex);
  const descContent = descMatch ? (descMatch[1] ?? descMatch[2] ?? descMatch[3] ?? descMatch[4] ?? "") : "";
  if (descContent) {
